...
RSpec.describe Shortener do
  ...
  it "shortens a full URL, receive response with the shortened URL" do
    ...
    shortened = Shortener.shorts_url(full_url,"http://domain.com")

    expect(shortened).to eq(short_url)
  end

  it "gives each URL its own short_url" do
    code_1 = Shortener.shorts_url(full_url, "http://domain.com")

    code_2 = Shortener.shorts_url(full_url2,"http://domain.com")

    expect(code_2).not_to eq(code_1)
  end

  it "find full_url by short url" do
    Shortener.shorts_url(full_url,"http://domain.com")
